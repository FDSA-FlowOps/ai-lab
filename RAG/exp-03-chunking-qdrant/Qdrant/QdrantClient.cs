...
namespace Exp03ChunkingQdrant.Qdrant;
...
public sealed class QdrantClient : IDisposable
{
    ...
    public async Task DeleteCollectionIfExistsAsync(string collection, CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new InvalidOperationException(
                $"Error al borrar coleccion '{collection}': {(int)response.StatusCode} {response.ReasonPhrase}. {TrimBody(body)}");
        }
    }

    public async Task CreateCollectionAsync(string collection, int vectorSize, CancellationToken cancellationToken)
    {
        var req = new CreateCollectionRequest(new VectorParams(vectorSize, "Cosine"));
        var response = await SendPutAsJsonAsync($"/collections/{collection}", req, cancellationToken);
        if (!response.IsSuccessStatusCode)
        {
            var body = await response.Content.ReadAsStringAsync(cancellationToken);
            throw new InvalidOperationException(
                $"Error al crear coleccion '{collection}': {(int)response.StatusCode} {response.ReasonPhrase}. {TrimBody(body)}");
        }
    }
    ...
    public async Task EnsureCollectionAsync(string collection, int expectedVectorSize, CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            var body = await response.Content.ReadAsStringAsync(cancellationToken);
            throw new InvalidOperationException(
                $"Error al consultar coleccion '{collection}': {(int)response.StatusCode} {response.ReasonPhrase}. {TrimBody(body)}");
        }
        ...
        {
            throw new InvalidOperationException(
                $"Vector size incompatible. Coleccion='{currentSize.Value}', embedding='{expectedVectorSize}'. Ejecuta Reset collection.");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var request = new UpsertRequest(points);
        ...
        if (!response.IsSuccessStatusCode)
        {
            var body = await response.Content.ReadAsStringAsync(cancellationToken);
            throw new InvalidOperationException(
                $"Error upsert en coleccion '{collection}': {(int)response.StatusCode} {response.ReasonPhrase}. {TrimBody(body)}");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            var body = await response.Content.ReadAsStringAsync(cancellationToken);
            throw new InvalidOperationException(
                $"Error en query a coleccion '{collection}': {(int)response.StatusCode} {response.ReasonPhrase}. {TrimBody(body)}");
        }
        ...
        foreach (var item in payload.Result)
        {
            ...
            {
                Score = item.Score,
                DocId = GetPayloadString(p, "doc_id"),
                SectionTitle = GetPayloadString(p, "section_title"),
                ChunkIndex = GetPayloadInt(p, "chunk_index"),
                ChunkText = GetPayloadString(p, "chunk_text")
            });
        }
    ...
    {
        var req = new CountRequest { Exact = true };
        var response = await SendPostAsJsonAsync($"/collections/{collection}/points/count", req, cancellationToken);
        if (!response.IsSuccessStatusCode)
        {
            var body = await response.Content.ReadAsStringAsync(cancellationToken);
            throw new InvalidOperationException(
                $"Error consultando cantidad de puntos en '{collection}': {(int)response.StatusCode} {response.ReasonPhrase}. {TrimBody(body)}");
        }
