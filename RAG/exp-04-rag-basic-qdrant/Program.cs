...
using Exp04RagBasicQdrant.Qdrant;
using Exp04RagBasicQdrant.Services;
...
try
{
    ...
    var rag = new RagAnswerService(config, retrieval, chatClient);

    await RunMenuAsync(config, runtime, ingestion, retrieval, rag, qdrantClient, cts.Token);
    return 0;
}
    ...
    while (!cancellationToken.IsCancellationRequested)
    {
        ...
        try
        {
            switch (option)
            {
                ...
                case "3":
                    await AskQuestionAsync(config, runtime, rag, cancellationToken);
                    break;
                case "4":
...
static async Task AskQuestionAsync(
    AppConfig config,
    RuntimeSettings runtime,
    RagAnswerService rag,
    CancellationToken cancellationToken)
{
    ...
    {
        Console.WriteLine();
        Console.WriteLine("Debug - Chunks recuperados:");
...
{
    Console.WriteLine("=== EXP-04 RAG Basic + Qdrant ===");
    ...
    Console.WriteLine("3) Ask a question (RAG)");
    Console.WriteLine("4) Show retrieved chunks");
